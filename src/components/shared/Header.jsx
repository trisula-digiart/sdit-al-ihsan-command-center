    ...
    <header className="print:hidden bg-white border-b border-slate-200 h-16 px-6 flex items-center justify-between sticky top-0 z-20 shadow-sm">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative">
          ...
          {notificationsOpen && (
            <div className="absolute right-0 mt-2 w-80 md:w-96 bg-white border border-slate-200 rounded-2xl shadow-2xl py-2 z-30 animate-in fade-in slide-in-from-top-2 duration-150">
                ...
                {notifications.length > 0 ? (
                  notifications.map((item) => (
                    <div
                      ...
                      className={`p-3.5 text-xs hover:bg-slate-50 cursor-pointer transition-colors flex items-start gap-3 relative group ${
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 pr-4">
                        <div className="flex items-center justify-between">
                          <p className={`font-bold ${item.unread ? 'text-slate-900' : 'text-slate-600'}`}>
                            {item.title}
                          </p>
                          {item.unread && (
              ...
              {notifications.length > 0 && (
                <div className="px-4 py-2 border-t border-slate-100 flex items-center justify-between bg-slate-50/50">
                  <button
                    ...
                    className="text-[11px] text-emerald-600 font-semibold hover:underline flex items-center gap-1.5 disabled:opacity-50 disabled:no-underline"
                  >
